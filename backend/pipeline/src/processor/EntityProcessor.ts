...
export class EntityProcessor {
    ...
    constructor(convexUrl: string) {
        ...
        if (!this.secret) {
            throw new Error("CONVEX_API_KEY environment variable is required for EntityProcessor");
        }
    }
    ...
    private async processJob(job: Job<ProcessJobData>): Promise<void> {
        const { tenantId, integrationId, dataSourceId, entityType, entities, syncId, startedAt, metrics: previousMetrics } = job.data;
        ...
        try {
            ...
            await this.triggerLinking(integrationId, tenantId, dataSourceId, syncId, startedAt, entityType);
    ...
    private async processChunk(
        ...
        const existingMap = new Map(
            existingEntities.map(e => [e.externalId, e])
        );
