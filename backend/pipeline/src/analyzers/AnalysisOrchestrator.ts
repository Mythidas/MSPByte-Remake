...
export class AnalysisOrchestrator {
    ...
    constructor(convexUrl: string, analyzers: BaseAnalyzer[]) {
        ...
        if (!this.secret) {
            throw new Error("CONVEX_API_KEY environment variable is required for AnalysisOrchestrator");
        }
    }
    ...
    private async handleAnalyzeJob(job: Job<AnalyzeJobData>): Promise<void> {
        const { tenantId, integrationId, dataSourceId, syncId, entityType, startedAt, metrics: previousMetrics } = job.data;
        ...
        try {
            ...
            const context = await this.loadContext(tenantId, integrationId, dataSourceId, syncId);
            ...
            if (allAlerts.length > 0) {
                await this.processAlerts(allAlerts, tenantId, integrationId, dataSourceId, syncId);
            }
            ...
            const action = entityType ? `sync.${entityType}` : `sync.${integrationId}`;
    ...
    private async loadContext(
        ...
        const context: AnalysisContext = {
            ...
            getChildEntities(parentId: Id<"entities">): Entity[] {
                ...
                return childIds.map((id) => entityMap.get(id)).filter((e) => e !== undefined) as Entity[];
            },

            getParentEntity(childId: Id<"entities">): Entity | undefined {
                const parentRel = relationships.find((r) => r.childEntityId === childId);
    ...
    private mergeTags(results: AnalyzerResult[]): Map<Id<"entities">, string[]> {
        ...
        return new Map(Array.from(merged.entries()).map(([id, tagSet]) => [id, Array.from(tagSet)]));
    }
     ...
     */
    private mergeStates(results: AnalyzerResult[]): Map<Id<"entities">, EntityState> {
