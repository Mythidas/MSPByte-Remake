...
export class Microsoft365Adapter extends BaseAdapter {
    ...
    constructor(convexUrl: string) {
        ...
        if (!this.secret) {
            throw new Error("CONVEX_API_KEY environment variable is required for Microsoft365Adapter");
        }
    }
    ...
    protected async fetchData(jobData: SyncJobData): Promise<AdapterFetchResult> {
        ...
        if (!healthy) {
            throw new Error(`Connector health check failed for data source ${dataSourceId}`);
        }
        ...
        const entities = await Promise.all(
            groups.map(async (rawData: any) => {
                ...
                const parentGroupIds = parentGroups
                    ? parentGroups.filter((p) => p["@odata.type"] === "#microsoft.graph.group").map((p) => p.id)
                    : [];
    ...
    private async handlePolicySync(connector: Microsoft365Connector): Promise<AdapterFetchResult> {
        const { data: policies, error: policiesError } = await connector.getConditionalAccessPolicies();
