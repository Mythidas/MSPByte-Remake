...
export class Microsoft365Linker extends BaseLinker {
    ...
    protected async link(
        ...
        relationships.push(...this.linkGroupMemberships(entitiesByType.groups, entityMap));

        // 2. Link role assignments (identities → roles)
        relationships.push(...this.linkRoleAssignments(entitiesByType.roles, entityMap));

        // 3. Link license assignments (identities → licenses)
        relationships.push(...this.linkLicenseAssignments(entitiesByType.identities, entityMap));

        // 4. Link nested groups (groups → parent groups)
        relationships.push(...this.linkNestedGroups(entitiesByType.groups, entityMap));
            ...
            for (const memberId of members) {
                ...
                relationships.push({
                    ...
                    metadata: {
                        ...
                        memberDisplayName: member.rawData?.displayName || member.rawData?.userPrincipalName,
                    },
                });
            }
        }
            ...
            for (const licenseAssignment of assignedLicenses) {
                ...
                relationships.push({
                    ...
                    metadata: {
                        ...
                        userDisplayName: identity.rawData?.displayName || identity.rawData?.userPrincipalName,
