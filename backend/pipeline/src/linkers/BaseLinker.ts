...
export abstract class BaseLinker {
    ...
    constructor(convexUrl: string, integrationId: IntegrationId) {
        ...
        if (!this.secret) {
            throw new Error("CONVEX_API_KEY environment variable is required for BaseLinker");
        }
    }
    ...
    private async handleLinkJob(job: Job<LinkJobData>): Promise<void> {
        const { tenantId, integrationId, dataSourceId, syncId, startedAt, metrics: previousMetrics } = job.data;
        ...
        try {
            ...
            this.metrics.trackQuery();
            const existingRelationships = await this.convex.query(api.helpers.orm.list_s, {
            ...
            const desiredRelationships = await this.link(entities as any[], dataSourceId);
            ...
            await this.triggerAnalyze(tenantId, integrationId, dataSourceId, syncId, startedAt);
