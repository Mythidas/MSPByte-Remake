...
export class Microsoft365Linker extends BaseLinker {
   ...
   */
  private async handleGroupLinking(
    event: ProcessedEventPayload,
  ): Promise<void> {
    const {
      eventID,
      tenantID,
    ...
    try {
      ...
      const existingRelationshipsMap =
        await this.batchLoadRelationshipsByParents(
          groupIds,
      ...
      for (const group of groups) {
        ...
        const groupRelationships =
          existingRelationshipsMap.get(group._id) || [];
  ...
  private async handleRoleLinking(event: ProcessedEventPayload): Promise<void> {
    ...
    try {
      ...
      for (const role of roles) {
        const { data: members, error } = await connector.getRoleMembers(
          role.externalId,
        );
        ...
        for (const member of members) {
          ...
          if (identity && !identity.deletedAt) {
            ...
            const relationshipExists = existingRelationships.some(
              (rel: any) =>
                rel.childEntityId === identity._id &&
                rel.relationshipType === "assigned_role",
            );
   ...
   */
  private async computeAdminTags(
    tenantID: Id<"tenants">,
    roles: Doc<"entities">[],
   ...
   */
  private async handlePolicyLinking(
    event: ProcessedEventPayload,
  ): Promise<void> {
    const {
      eventID,
          ...
          for (const userId of includeUsers) {
            ...
            if (identity && !identity.deletedAt) {
              ...
              const relationshipExists = existingRelationships.some(
                (rel: any) =>
                  rel.childEntityId === identity._id &&
                  rel.relationshipType === "applies_to",
              );
   ...
   */
  private async handleIdentityLicenseLinking(
    event: ProcessedEventPayload,
  ): Promise<void> {
    const {
      eventID,
        ...
        for (const licenseSkuId of licenseSkus) {
          ...
          if (license && !license.deletedAt) {
            ...
            const relationshipExists = existingRelationships.some(
              (rel: any) =>
                rel.childEntityId === identity._id &&
                rel.relationshipType === "has_license",
            );
   ...
   */
  private async handleLicenses(
    processedEvent: ProcessedEventPayload,
  ): Promise<void> {
    Logger.log({
      module: "Microsoft365Linker",
   ...
   */
  private async publishLinkedEvent(
    processedEvent: ProcessedEventPayload,
  ): Promise<void> {
