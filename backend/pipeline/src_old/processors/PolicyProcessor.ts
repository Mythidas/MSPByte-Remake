import { BaseProcessor, PolicyData } from "./BaseProcessor.js";
...
export class PolicyProcessor extends BaseProcessor {
      ...
      if (row.externalID === "security-defaults") {
        ...
        return {
          ...
          normalized: {
            ...
            description:
              "Azure AD Security Defaults provide baseline security policies",
  ...
  private createPolicyDescription(
    ...
    if (policy.conditions?.users) {
      ...
      if (includeUsers?.includes("All")) {
        ...
        parts.push(
          `Targets ${includeUsers?.length || 0} users and ${includeGroups?.length || 0} groups`,
        );
      }
    }
