  ...
  handler: async (ctx, args) => {
    ...
    if (existingDataSource) {
      ...
      await ctx.db.patch(existingDataSource._id, {
        config: args.config,
        credentialExpirationAt:
          args.credentialExpirationAt ||
          existingDataSource.credentialExpirationAt,
        status: "active",
        updatedAt: new Date().getTime(),
      });
...
export const deleteAllData = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const [entities, relationships, alerts, mappings] = await Promise.all([
      ctx.db
        .query("entities")
        .withIndex("by_data_source", (q) => q.eq("dataSourceId", args.id))
        .collect(),
      ctx.db
        .query("entity_relationships")
        .withIndex("by_data_source_type", (q) => q.eq("dataSourceId", args.id))
        .collect(),
      ctx.db
        .query("entity_alerts")
        .withIndex("by_data_source", (q) => q.eq("dataSourceId", args.id))
