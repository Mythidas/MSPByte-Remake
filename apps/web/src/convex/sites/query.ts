...
export const getSiteWithIntegrationsView = query({
	...
	handler: async (ctx, args) => {
		...
		const allDataSources = [
			...directDataSources,
			...linkedDataSources.filter((ds): ds is NonNullable<typeof ds> => ds !== null)
		];

		// Get unique integration IDs
		const uniqueIntegrationIds = [...new Set(allDataSources.map((ds) => ds.integrationId))];

		// Fetch all integrations in parallel
		const integrations = await Promise.all(uniqueIntegrationIds.map((id) => ctx.db.get(id)));
		...
		return {
			...site,
			psaIntegrationName: integrations.find((i) => i?._id === site.psaIntegrationId)?.name,
			linkedIntegrations
		};
	}
});
