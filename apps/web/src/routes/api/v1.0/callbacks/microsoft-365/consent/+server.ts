...
export const GET: RequestHandler = async ({ locals, url }) => {
	...
	try {
		...
		const dataSource = await locals.client.mutation(api.datasources.crud.create_s, {
			secret: CONVEX_API_KEY,
		...
		redirect(307, `${redUrl}?success=true&tenant=${encodeURIComponent(tenantInfo.displayName)}`);
	} catch (error) {
		console.log(error);
