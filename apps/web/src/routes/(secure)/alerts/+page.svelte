<script lang="ts">
	...
	import AlertDetailSheet from '../sites/[slug]/integrations/microsoft-365/helpers/AlertDetailSheet.svelte';
	...
	let statusIndex = $state<string | undefined>(undefined);
	...
	const alertsQuery = useQuery(api.helpers.orm.list, () => ({
		...
		filters: filters,
		index: {
			name: 'by_status' as const,
			params: {
				status: statusIndex || 'active'
			}
		}
	}));
	...
	const sitesQuery = useQuery(api.helpers.orm.list, () => ({
		tableName: 'sites' as const
	}));

	const rawAlerts = $derived((alertsQuery.data || []) as Doc<'entity_alerts'>[]);
	const sites = $derived((sitesQuery.data || []) as Doc<'sites'>[]);

	const isLoading = $derived(alertsQuery.isLoading || sitesQuery.isLoading);

	// Client-side join: enrich alerts with site and group information
	const alerts = $derived.by(() => {
		// Create lookup maps for efficient joins
		const siteMap = new Map(sites.map((s) => [s._id, s]));

		// Create site -> groups lookup
		// Enrich alerts with site and group data
		return rawAlerts.map((alert) => {
			const site = alert.siteId ? siteMap.get(alert.siteId) : null;
...
<div class="flex size-full flex-col gap-4 overflow-hidden p-6">
	<div class="flex w-full justify-between">
		...
		<div>
			<Select.Root type="single" onValueChange={(v) => (statusIndex = v)}>
				<Select.Trigger>{prettyText(statusIndex || 'Select status')}</Select.Trigger>
				<Select.Content>
