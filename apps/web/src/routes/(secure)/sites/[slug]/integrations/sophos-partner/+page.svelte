...
<div class="flex size-full flex-col gap-4">
	<h1 class="text-2xl">Sophos Devices</h1>
	...
	<DataTable
		...
		columns={[
			...
			},
			{
				...
				sortable: true,
				render: ({ row }) =>
					(row.rawData as any)?.tamperProtectionEnabled ? 'Enabled' : 'Disabled'
			},
			{
			...
			},
			{
				...
				sortable: true,
				render: ({ row }) =>
					new Date((row.normalizedData as any).last_check_in).toLocaleDateString()
			}
		]}
	/>
</div>
