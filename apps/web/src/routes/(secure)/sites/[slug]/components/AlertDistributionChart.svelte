<script lang="ts">
	...
	type Props = {
		...
		medium: number;
		low: number;
		onSegmentClick?: (severity: AlertSeverity) => void;
	};

	let { critical = 0, high = 0, medium = 0, low = 0, onSegmentClick }: Props = $props();
				...
				<Pie let:arcs>
					{#each arcs as arc}
						<Arc
							...
							on:click={() => onSegmentClick?.(arc.data.severity)}
						/>
					{/each}
				</Pie>
			</Svg>
	...
	<div class="mt-4 flex flex-wrap justify-center gap-4 text-sm">
		{#each chartData as item}
			<button
				...
				onclick={() => onSegmentClick?.(item.severity)}
			>
