...
export function IntegrationHeader({
  ...
  const breadcrumbs = useMemo(() => {
    ...
    const integrationIndex = pathSegments.findIndex(
      (seg) => seg === "integrations",
    );
    ...
    return afterIntegration.map((segment, index) => {
      const path =
        "/" + pathSegments.slice(0, integrationIndex + 2 + index + 1).join("/");
      return {
  ...
  return (
    <div className="flex h-12 p-4 bg-card/50 border rounded shadow items-center w-full justify-between">
      <Breadcrumb>
        <BreadcrumbList>
          <BreadcrumbLink href="/secure/default/integrations">
            Integrations
          </BreadcrumbLink>
          <BreadcrumbSeparator />
          <BreadcrumbLink
            href={`/secure/default/integrations/${integration.slug}`}
          >
            {integration.name}
          </BreadcrumbLink>
      ...
      <div className="flex gap-2">
        ...
        {CustomActions ? (
          ...
          <Button
            variant="destructive"
            className="px-2 h-7 hover:cursor-pointer"
          >
            Disable
          </Button>
        )}
