...
export function DataTableFilterChips<TData>({
    ...
    const viewFilterFields = new Set(viewFilters.map((f) => `${f.field}:${f.operator}:${JSON.stringify(f.value)}`));

    // Only show dynamic filters (not from views)
    const dynamicFilters = filters.filter(
        (f) => !viewFilterFields.has(`${f.field}:${f.operator}:${JSON.stringify(f.value)}`)
    );
        ...
        if (column?.filter?.type === "select" && column.filter.options) {
            if (Array.isArray(filter.value)) {
                const labels = filter.value
                    .map((v) => column.filter?.options?.find((opt) => opt.value === v)?.label || v)
                    .join(", ");
                valueLabel = labels;
            } else {
                const option = column.filter.options.find((opt) => opt.value === filter.value);
    ...
    return (
        <div className="flex flex-wrap items-center gap-2">
            {dynamicFilters.map((filter, index) => (
                <Badge key={`${filter.field}-${index}`} variant="secondary" className="gap-1">
