...
export default function SitesPage() {
	...
	const sites = useQuery(api.helpers.orm.list, { tableName: "sites" }) as
		| Site[]
		| undefined;
	...
	const columns: DataTableColumn<Site>[] = [
		{
			...
			searchable: true,
			cell: ({ row }) => (
				<Link href={`/secure/sites/${row.slug}`}>{row.name}</Link>
			),
			filter: {
				type: "text",
		...
		},
		{
			...
			sortable: true,
			cell: ({ row }) =>
				row.psaIntegrationId ? INTEGRATIONS[row.psaIntegrationId].name : "-",
			filter: {
				type: "text",
	...
	const rowActions: RowAction<Site>[] = [
		...
		},
		{
			...
			onClick: (rows) => {
				if (
					confirm(`Are you sure you want to delete ${rows.length} site(s)?`)
				) {
