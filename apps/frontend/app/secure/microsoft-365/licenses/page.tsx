...
import { AlertCircle, AlertTriangle, TrendingUp, Building2 } from "lucide-react";
...
export default function Microsoft365Licenses() {
    ...
    const columns: DataTableColumn<LicenseEntity>[] = [
        {
            ...
            sortable: true,
            searchable: true,
            cell: ({ row }) => row.normalizedData?.name || row.rawData?.skuPartNumber || '-',
            filter: {
                type: "text",
                ...
                return (
                    <div className="flex items-center gap-2">
                        <div className="w-24 bg-muted rounded-full h-2">
                            <div
                                className={`h-2 rounded-full transition-all ${isOverused
                                    ? 'bg-red-500'
                                    : isHighUsage
                                        ? 'bg-yellow-500'
                                        : 'bg-green-500'
                        ...
                        <span className={`text-sm font-medium ${isOverused ? 'text-red-500' : isHighUsage ? 'text-yellow-500' : ''
                            }`}>
                            {percentage}%
                        </span>
                    </div>
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                const tagConfig: Record<string, { color: string; icon: any }> = {
                    bloat: { color: "bg-yellow-500/50 border-yellow-500", icon: AlertTriangle },
                ...
                return (
                    <div className="flex flex-wrap gap-1">
                        {tags.map((tag: string) => {
                            const config = tagConfig[tag.toLowerCase()] || { color: "bg-muted border-border", icon: null };
    ...
    return (
        <div className="flex flex-col size-full gap-2">
            <div>
                <h1 className="text-3xl font-bold tracking-tight">Microsoft 365 Licenses</h1>
