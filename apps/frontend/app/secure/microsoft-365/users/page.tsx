...
export default function Microsoft365Users() {
	...
	const columns: DataTableColumn<UserEntity>[] = [
		...
		},
		{
			...
			cell: ({ row }) => {
				...
				const config =
					stateConfig[state as keyof typeof stateConfig] || stateConfig.normal;
	...
	const views: TableView[] = [
		...
		},
		{
			...
			filters: [
				...
				{ field: "rawData.accountEnabled", operator: "eq", value: true },
			],
		},
	];
	...
	const hasActiveFilters =
		users && filteredData.length > 0 && filteredData.length < users.length;

	// Export handlers - use filtered data if available
	const handleExportCSV = () => {
		const dataToExport = (hasActiveFilters ? filteredData : users || []).map(
			(user) => {
				return {
					...user,
				};
		...
		toast.success(
			`Exported ${dataToExport.length} user${dataToExport.length !== 1 ? "s" : ""} to CSV`,
		);
	};
	...
	const handleExportJSON = () => {
		...
		toast.success(
			`Exported ${dataToExport.length} user${dataToExport.length !== 1 ? "s" : ""} to JSON`,
		);
	};
	...
	const handleBulkEnable = (selectedRows: UserEntity[]) => {
		toast.info(
			`Bulk enable functionality coming soon (${selectedRows.length} users selected)`,
		);
	...
	const handleBulkDisable = (selectedRows: UserEntity[]) => {
		toast.info(
			`Bulk disable functionality coming soon (${selectedRows.length} users selected)`,
		);
	...
	const handleBulkResetPassword = (selectedRows: UserEntity[]) => {
		toast.info(
			`Bulk password reset functionality coming soon (${selectedRows.length} users selected)`,
		);
	...
	const handleBulkRevokeSessions = (selectedRows: UserEntity[]) => {
		toast.info(
			`Bulk revoke sessions functionality coming soon (${selectedRows.length} users selected)`,
		);
	...
	const handleBulkLockdown = (selectedRows: UserEntity[]) => {
		toast.info(
			`Bulk lockdown functionality coming soon (${selectedRows.length} users selected)`,
		);
