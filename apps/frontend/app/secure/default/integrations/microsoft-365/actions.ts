    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Failed to generate consent URL",
    };
  }
}
...
export async function enableIntegration() {
  try {
    ...
    const dataSource = await client.mutation(
 ...
 */
export async function checkPermissionVersion(params: { dataSourceId: string }) {
  ...
  try {
    ...
    return {
      data: {
        needsReconsent: connectionVersion < PERMISSION_VERSION,
        currentVersion: PERMISSION_VERSION,
        connectionVersion,
      },
      error: null,
    };
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Failed to check permission version",
    };
  }
}
