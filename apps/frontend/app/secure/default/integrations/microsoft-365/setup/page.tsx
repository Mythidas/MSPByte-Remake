...
import { generateConsentUrl } from "../actions";
...
export default function Microsoft365Setup() {
  ...
  const [currentStep, setCurrentStep] = useState<
    "enable" | "primary-source" | "connection"
  >("enable");
  ...
  const handleEnable = async () => {
    ...
    try {
      ...
      toast.error(
        "Failed to enable integration: " + (error.message || "Unknown error"),
      );
    } finally {
      setIsEnabling(false);
    }
  ...
  const handleSetPrimarySource = async (sourceType: "m365" | "other") => {
    try {
      ...
      toast.success(
        `Primary data source set to ${sourceType === "m365" ? "Microsoft 365" : "Other"}`,
      );
      setCurrentStep("connection");
    } catch (error: any) {
      toast.error(
        "Failed to set primary source: " + (error.message || "Unknown error"),
      );
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 max-w-3xl mx-auto">
      ...
      <div className="flex gap-4">
        <div
          className={`flex items-center gap-2 ${currentStep === "enable" ? "text-primary" : isEnabled ? "text-green-400" : "text-muted-foreground"}`}
        >
          {isEnabled ? (
        ...
        <div
          className={`flex items-center gap-2 ${currentStep === "primary-source" ? "text-primary" : currentStep === "connection" ? "text-green-400" : "text-muted-foreground"}`}
        >
        ...
        <div
          className={`flex items-center gap-2 ${currentStep === "connection" ? "text-primary" : "text-muted-foreground"}`}
        >
          <div className="w-5 h-5 rounded-full border-2" />
      ...
      {currentStep === "enable" && (
        <div className="bg-card/50 border rounded shadow p-6 flex flex-col gap-4">
          ...
          <p className="text-muted-foreground">
            Enable the Microsoft 365 integration to start syncing users, groups,
            licenses, and more from your Entra tenants.
          </p>
      ...
      {currentStep === "primary-source" && (
        <div className="bg-card/50 border rounded shadow p-6 flex flex-col gap-4">
          ...
          <p className="text-muted-foreground">
            Choose whether Microsoft 365 should be your primary source for user
            data, or if you're using another integration as the primary source.
          </p>
      ...
      {currentStep === "connection" && (
        <div className="bg-card/50 border rounded shadow p-6 flex flex-col gap-4">
          ...
          <p className="text-muted-foreground">
            Create a connection to a Microsoft 365 tenant. You'll be redirected
            to Microsoft to grant admin consent for accessing tenant data.
          </p>
          ...
          <div className="flex flex-col gap-4 mt-2">
            ...
            <div className="bg-blue-500/10 border border-blue-500/20 rounded p-4 flex gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <span className="text-xs text-muted-foreground">
                  You'll need Global Administrator permissions in the Microsoft
                  365 tenant to grant the necessary permissions for this
                  integration.
                </span>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              ...
              <Button
                variant="outline"
                onClick={() =>
                  router.push("/secure/default/integrations/microsoft-365")
                }
                disabled={isRedirecting}
              >
                Skip for Now
      ...
      {isEnabled && primaryDataSource && (
        <div className="bg-green-500/10 border border-green-500/20 rounded p-4 flex gap-3">
          ...
          <div className="flex flex-col gap-1">
            ...
            <span className="text-xs text-muted-foreground">
              You can manage connections and domain mappings from the
              Connections page.
            </span>
          </div>
        </div>
