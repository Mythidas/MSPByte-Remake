...
export default function Microsoft365Connections() {
  ...
  const [connectionPermissions, setConnectionPermissions] = useState<
    Map<string, boolean>
  >(new Map());
  ...
  const primaryDataSource = useQuery(api.helpers.orm.get, {
  ...
  const dataSources = useQuery(api.helpers.orm.list, {
    tableName: "data_sources",
    index: {
      name: "by_integration",
      params: {
  ...
  const sites = useQuery(api.helpers.orm.list, {
    tableName: "sites",
  }) as Doc<"sites">[] | undefined;

  const dataSourceToSiteLinks = useQuery(api.helpers.orm.list, {
  ...
  const updateM365DomainMappingsMutation = useMutation(
    api.datasources.mutate.updateM365DomainMappings,
  );

  const isLoading =
    dataSources === undefined ||
    sites === undefined ||
    dataSourceToSiteLinks === undefined;
  ...
  useEffect(() => {
    if (selectedConnection) {
      checkPermissionVersion({ dataSourceId: selectedConnection._id }).then(
        (result) => {
          if (result.data) {
  ...
  const filteredConnections = useMemo(() => {
    if (filter === "needs-reconsent") {
      return searchFilteredConnections.filter(
        (conn) => connectionPermissions.get(conn._id) === true,
      );
    }
  ...
  const getMappedSite = (domain: string) => {
    ...
    return mapping && sites
      ? sites.find((s) => s._id === mapping.siteId)
      : undefined;
  };
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to save domain mappings",
      );
    } finally {
      setIsSaving(false);
    }
  ...
  const saveConnectionName = async () => {
    ...
    try {
      ...
      const updatedConfig = {
        ...selectedConnection.config,
        name: editedConnectionName.trim(),
      } as Microsoft365DataSourceConfig;
      setSelectedConnection({ ...selectedConnection, config: updatedConfig });
    } catch (error: any) {
      toast.error(
        "Failed to update name: " + (error.message || "Unknown error"),
      );
    } finally {
  ...
  const handleReconsent = async () => {
    ...
    try {
      ...
      toast.error(
        "Failed to start reconsent flow: " + (error.message || "Unknown error"),
      );
  ...
  const handleCreateConnection = async () => {
    ...
    try {
      ...
      toast.error(
        "Failed to start consent flow: " + (error.message || "Unknown error"),
      );
      setIsRedirecting(false);
    }
  };
  ...
  const isMappingsChanged = useMemo(() => {
    ...
    return (
      JSON.stringify(existing.domainMappings) !== JSON.stringify(domainMappings)
    );
    ...
    <div className="flex flex-col gap-4 size-full">
      ...
      >
        <TabsList>
          ...
          <TabsTrigger value="sites" disabled={!selectedConnection}>
            Connection Settings
          </TabsTrigger>
        </TabsList>
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Create a connection to a Microsoft 365 tenant. You'll be
                    redirected to Microsoft to grant admin consent.
                  </DialogDescription>
                </DialogHeader>
                <div className="flex flex-col gap-4 mt-4">
                  <div className="flex flex-col gap-2">
                    <Label htmlFor="dialogConnectionName">
                      Connection Name
                    </Label>
                    <Input
          ...
          <div className="flex-1 overflow-auto pr-3">
            <div className="grid gap-3">
              {filteredConnections.map((connection) => {
                const config =
                  connection.config as Microsoft365DataSourceConfig;
                const needsReconsent =
                  connectionPermissions.get(connection._id) === true;
                return (
                  <div
                    key={connection._id}
        ...
        >
          {!selectedConnection ? (
            ...
            <>
              {(() => {
                const config =
                  selectedConnection.config as Microsoft365DataSourceConfig;
                    ...
                    <div className="bg-card/50 border rounded shadow p-6 flex flex-col gap-4">
                          ...
                          {!isEditingName ? (
                            ...
                            <>
                              <Button
                                ...
                                disabled={
                                  isSavingName || !editedConnectionName.trim()
                                }
                                className="gap-2"
                              >
                      ...
                      <div className="flex items-center justify-between p-4 bg-red-500/10 border border-red-500/20 rounded">
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-muted-foreground">
                            Permanently remove this connection and all its
                            mappings
                          </span>
                        </div>
                        <Button
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-muted-foreground text-sm">
                            {domainMappings.length} of{" "}
                            {config.availableDomains?.length || 0} domains
                            mapped
                          </span>
                          <Button
                                ...
                                >
                                  ...
                                  <div className="flex gap-2 items-center min-w-[250px]">
                                    <SearchBox
                                      ...
                                      onSelect={(value) =>
                                        updateMapping(domain.name, value)
                                      }
                                      className="flex-1"
                                    />
                                    {mappedSite && (
                                      <Button
                                        ...
                                        size="icon"
                                        onClick={() =>
                                          updateMapping(domain.name, undefined)
                                        }
                                      >
                                        <X className="h-4 w-4" />
                                      </Button>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the connection and all its domain
