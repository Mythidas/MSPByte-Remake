...
import { Integration } from "@workspace/shared/types/integrations/config";
	...
	return (
		<div className="flex flex-col gap-2 size-full">
			...
			<div className="flex gap-2 size-full overflow-hidden">
				...
				<div className="flex size-full overflow-hidden">{children}</div>
			</div>
		</div>
	);
}
			...
			<BreadcrumbList>
				{parts.map((part, idx) => {
					...
					if (idx === parts.length - 1) {
						return (
							<BreadcrumbPage key={idx}>{prettyText(part)}</BreadcrumbPage>
						);
					}

					return (
						<React.Fragment key={idx}>
							<BreadcrumbLink href={getLink(idx)}>
								{prettyText(part)}
							</BreadcrumbLink>
							<BreadcrumbSeparator />
						</React.Fragment>
